...
def sheet_xml(rows, widths, freeze_rows=1, autofilter_end=None):
    ...
    pane = (
        f'<sheetViews><sheetView workbookViewId="0"><pane ySplit="{freeze_rows}" '
        f'topLeftCell="A{freeze_rows + 1}" activePane="bottomLeft" state="frozen"/>'
        f'<selection pane="bottomLeft" activeCell="A{freeze_rows + 1}" '
        f'sqref="A{freeze_rows + 1}"/></sheetView></sheetViews>'
        if freeze_rows else
    ...
    return (
        ...
        + pane + '<cols>' + ''.join(cols) + '</cols><sheetData>' + ''.join(body)
...
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/>
    ...
    workbook = (
        ...
        + ''.join(
            ...
            for i, (name, _, _, _) in enumerate(sheet_specs, 1)
        )
        + '</sheets></workbook>'
    )
    ...
    # cellXfs: 0 default, 1 header, 2 text, 3 integer, 4 decimal,
    # 5 percentage, 6 datetime, 7 bold, 8 title.
    # IMPORTANT: percentage cells must use style 5 / numFmtId 166.
...
<xf numFmtId="166" fontId="0" fillId="0" borderId="1"/>
<xf numFmtId="167" fontId="0" fillId="0" borderId="1"/>
    ...
    with zipfile.ZipFile(path, 'w', compression=zipfile.ZIP_DEFLATED) as z:
        ...
        for i, (_, rows, widths, end_ref) in enumerate(sheet_specs, 1):
            z.writestr(
                ...
                sheet_xml(rows, widths, freeze_rows=1, autofilter_end=end_ref)
            )
...
def pct_cell(value):
    # Source data uses percentage-point values: 3.2 means 3.2%.
    # Excel percentage format expects the decimal form: 0.032.
    return (num(value) / 100.0, 5, 'number')
...
def main():
    ...
    enriched.sort(
        key=lambda x: (
            pool_order.get(x[0].get('pool'), 9),
            -num(x[0].get('adjusted_total'))
        )
    )
    ...
    for i, (r, entered, rec, hist, res, scores, watch) in enumerate(enriched, 1):
        rows.append([
            ...
            num_cell(num(r.get('adjusted_total')), 4),
            num_cell(num(scores.get('total')), 4),
            style_value('P' + text(rec.get('tier')), 2),
            style_value(rec.get('tag'), 2),
            style_value(r.get('pool'), 2),
            style_value(r.get('grade'), 2),
            num_cell(num(res.get('count')), 3),
            pct_cell(r.get('lianban_probability')),
            num_cell(num(hist.get('limit_up_count')), 3),
            num_cell(num(hist.get('max_consecutive')), 3),
            num_cell(num(hist.get('days_since_last_lu')), 3),
            style_value(watch[0] if len(watch) > 0 else '-', 2),
            style_value(watch[1] if len(watch) > 1 else '-', 2)
        ])
    ...
    for p in ['重点观察', '预备池', '观察池', '淘汰']:
        overview.append([
            style_value(p, 2),
            num_cell(sum(1 for r in candidates if r.get('pool') == p), 3)
        ])
    overview += [
        [],
        [style_value('行业排名 TOP 15', 8), style_value('涨跌幅', 8), style_value('涨停', 8), style_value('强势', 8)]
    ]
    for x in sorted(data.get('sector_rankings') or [], key=lambda x: num(x.get('rank')))[:15]:
        overview.append([
            style_value(x.get('name'), 2),
            pct_cell(x.get('avg_change')),
            num_cell(num(x.get('limit_up_count')), 3),
            num_cell(num(x.get('strong_count')), 3)
        ])
    overview += [
        [],
        [style_value('概念排名 TOP 15', 8), style_value('涨跌幅', 8), style_value('涨停', 8), style_value('强势', 8)]
    ]
