#!/usr/bin/env python3
"""Render the full candidate pool and maintain persistent pool-entry timestamps."""
import html
import json
import os
...
HISTORY = os.path.join(ROOT, "data", "candidate_pool_history.json")
...
def update_pool_history(candidates, scan_time):
    ...
    for r in candidates:
        ...
        r["pool_entry_time"] = entered_at

    for code, item in history.items():
        if code not in current_codes and isinstance(item, dict):
            item["current_pool"] = None
...
def main():
    ...
    scan_time = str(data.get("scan_time") or data.get("scan_date") or "")
    update_pool_history(candidates, scan_time)
    ...
    sectors = sorted({str(r.get("sector") or r.get("sw_industry") or "-") for r in ranked})
    ...
    section = (
        ...
        '<th>入池时间</th><th>代码</th><th>名称</th><th>行业</th><th>价格</th><th>涨跌幅</th><th>换手率</th><th>流通市值</th><th>调整分 / 原始分</th><th>P级</th><th>候选池</th><th>评级</th><th>三共振</th><th>连板概率</th><th>历史涨停</th><th>距上次</th><th>次日重点</th>'
