...
def factors(r):
    ...
    for name,maxv,key in cfg:
        try:
            value=float(scores.get(key,0)); width=max(0,min(100,value/maxv*100)); val=f"{value:.1f}"
        except Exception:
            width=0; val="-"
...
def stock_card(r, i):
    ...
    return f'''<article class="stock-card" data-pool="{esc(pool)}" data-grade="{esc(grade)}" data-tier="{esc(rec.get("tier","99"))}" data-name="{esc(r.get("name",""))} {esc(r.get("code",""))} {esc(r.get("sector",""))}">
      <div class="stock-top"><div class="rank">#{i}</div><div><div class="stock-name">{esc(r.get("name"))} <span>{esc(r.get("code"))}</span></div><div class="stock-meta"><span>{esc(r.get("sector","未分类"))}</span><span>{esc(r.get("sw_industry","未分类"))}</span><span>{esc(rec.get("tag","-"))}</span></div></div><div class="price-box"><strong>{esc(r.get("price"))}</strong><span class="{pct_cls(r.get("change_pct"))}">{pct(r.get("change_pct"))}</span></div><div class="score-box"><b>{esc(score)}</b><span>/100</span><div class="score-line"><i style="width:{score_w:.0f}%"></i></div><small>原始 {esc(r.get("scores",{}).get("total","-"))}</small></div><div class="labels"><span class="pill {pool_cls(pool)}">{esc(pool)}</span><span class="pill grade-{grade_cls(grade)}">{esc(grade)}</span><span class="pill tier">P{esc(rec.get("tier","-"))}</span></div><button class="detail-btn" onclick="toggleDetail(this)">展开详情</button></div>
      <div class="quick-grid"><div><span>三共振</span><b class="resonance">{dots}</b><em>{rc}/3</em></div><div><span>连板概率</span><b>{esc(r.get("lianban_probability","-"))}%</b></div><div><span>涨停历史</span><b>{esc(hist.get("limit_up_count","-"))}次</b><em>最高{esc(hist.get("max_consecutive","-"))}连</em></div><div><span>距上次涨停</span><b>{esc(hist.get("days_since_last_lu","-"))}日</b></div><div><span>换手率</span><b>{esc(r.get("turnover_rate","-"))}%</b></div><div><span>流通市值</span><b>{esc(r.get("circ_mcap_yi","-"))}亿</b></div></div>
...
document.getElementById('search').addEventListener('input',render);document.getElementById('sort').addEventListener('change',render);document.getElementById('grade').addEventListener('change',render);document.querySelectorAll('.filter-btn').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.filter-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');pool=b.dataset.pool;render()}));render();
    ...
    return html_tpl.substitute(MODEL=esc(data.get("model","")),DATE=esc(data.get("scan_date","")),TIME=esc(data.get("scan_time","")),MOOD=mood(sent),LABEL=esc(sent.get("sentiment_label","-")),SCORE=esc(sent.get("sentiment_score","-")),LUP=esc(sent.get("limit_up_count","-")),LDN=esc(sent.get("limit_down_count","-")),STRONG=esc(sent.get("strong_count","-")),EXPLODE=esc(sent.get("explosion_rate","-")),MAXBOARD=esc(sent.get("max_boards_est","-")),ICE="是" if sent.get("is_ice_point") else "否",TOTAL=esc(summary.get("total_scanned",len(candidates))),FOCUS_COUNT=esc(pools.get("重点观察",0)),READY_COUNT=esc(pools.get("预备池",0)),WATCH_COUNT=esc(pools.get("观察池",0)),FAILED=esc(summary.get("total_failed",0)),SECTOR_ROWS=sector_rows or '<tr><td colspan="5" class="muted">暂无数据</td></tr>',CONCEPTS=concept_tags or '<span class="muted">暂无概念数据</span>',WEIGHTS=weights,THRESHOLDS=thresholds,CARDS=cards,VERSION=esc(data.get("system_version","")))
